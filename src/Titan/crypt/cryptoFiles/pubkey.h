...
{
	H h;
	SecByteBlock buf((std::max)(4U, (unsigned int)H::DIGESTSIZE));
	word32 counter = 0;
	...
	while (outputLength)
	{
		...
		unsigned int xorLen = (std::min)((unsigned int)H::DIGESTSIZE, outputLength);
...
protected:
	...
	unsigned int PaddedBlockBitLength() const {return this->f.MaxImage().BitCount()-1;}
};
...
{
	std::unique_ptr<HashModule> ma(messageAccumulator);
...
{
	std::unique_ptr<H> ma(static_cast<H*>(messageAccumulator));
	if (ma->MaximumRecoverableLength() == 0)
		throw KeyTooShort();
	SecByteBlock representative(this->PaddedBlockByteLength());
	ma->Encode(rng, representative);
	this->f.CalculateInverse(Integer(representative, representative.size)).Encode(signature, SignatureLength());
}
