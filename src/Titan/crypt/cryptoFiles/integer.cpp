...
{
	unsigned int outputLen = (std::max)(1U, ByteCount());
...
unsigned int Integer::Encode(byte *output, unsigned int outputLen, Signedness signedness) const
{
	...
	else
	{
		...
		Integer temp = Integer::Power2(8*(std::max)(ByteCount(), outputLen)) + *this;
		for (unsigned i=0; i<outputLen; i++)
	...
	switch (rnType)
	{
		...
		case PRIME:
			...
			while (1)
			{
				...
				Randomize(rng, min, max);
				if (FirstPrime(*this, (std::min)(*this+mod*PrimeSearchInterval(max), max), equiv, mod))
					return true;
			}
...
{
	Integer sum((word)0, (std::max)(a.reg.size, b.reg.size));
	if (a.NotNegative())
	{
		if (b.NotNegative())
			PositiveAdd(sum, a, b);
...
Integer operator-(const Integer &a, const Integer& b)
{
	Integer diff((word)0, (std::max)(a.reg.size, b.reg.size));
	if (a.NotNegative())
	{
		if (b.NotNegative())
