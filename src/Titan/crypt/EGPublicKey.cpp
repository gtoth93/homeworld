                         ...
                         unsigned long theLen) const
{
	...
	unique_ptr<unsigned char> aBuf(new unsigned char[mCryptP->CipherTextLength()]);
...
EGPublicKey::Encrypt(const void* theMsgP, unsigned long theLen) const
{
	...
	try
	{
		...
		unique_ptr<unsigned char> anOutP(new unsigned char [anOutLen]);
	                ...
	                const void* theMsgP, unsigned long theMsgLen) const
{
	...
	try
	{
		...
		unique_ptr<HashModule> aHashP(mSigP->NewMessageAccumulator());
