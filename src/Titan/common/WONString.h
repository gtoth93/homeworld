...
private:
    ...
    void MakeTitanWide() const
    {
        MakeWide();
#ifdef WIN32
        std::basic_string<wchar> tmp{};
        for (auto c: wstr) {
            tmp += *reinterpret_cast<wchar*>(&c);
        }
        titanwstr.assign(tmp);
#else
    ...
    int wstr_compare(const wchar* s, size_t pos, size_t n) const		{ return wstr.compare(0, 0xFFFFFFFF, reinterpret_cast<const wchar_t*>(s), pos, n); }
