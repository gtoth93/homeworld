...
// Default constructor
ThreadData::ThreadData(const char* theName) : mName(),
...
// Copy constructor
ThreadData::ThreadData(const ThreadData&theData) : mName(theData.mName),
...
// Default constructor
Tracer::Tracer(const string&theMethod) : mMethod(theMethod) {
    ThreadData&aTDataR = GetThreadData(GetCurrentThreadId());
