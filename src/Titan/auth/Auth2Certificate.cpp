				...
				int                   &theLength )
{
	...
	unique_ptr<CertificateDataBlock> aBlkAP;
	switch( aContentType )
	{
	case CTAuthData:
		aBlkAP = unique_ptr<CertificateDataBlock>(new AuthDataBlk);
		break;
	}
...
void Auth2Certificate::FreeDataList()
{
    ...
    {
        unique_ptr<CertificateDataBlock> aDataBlock( mDataList.front() );
        mDataList.pop_front();
    }
}
