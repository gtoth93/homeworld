...
struct Profile2GetReplyInfo
{
	...
	{ str.assign((const wchar_t*)(mBuffer.data()), mBuffer.size() / 2); }

	WONCommon::WONString GetString(void) const
	{ return WONCommon::WONString((const wchar_t*)(mBuffer.data()), mBuffer.size() / 2); }
...
public:
	...
	bool GetStringField(int i, WONCommon::WONString& str) const
	{
		...
		if (buf)
		{
			str.assign((const wchar_t*)(buf->data()), buf->size() / 2);
			return true;
		}
		return false;
	}
	...
	bool GetNewsletterSub(WONCommon::WONString& str, int j = 0)	 const { 
		...
		return p ? (str.assign((const wchar_t*)(p->data()), p->size() / 2), true) : false;
	}
